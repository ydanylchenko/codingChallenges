...
//        return 20 * IntStream.range(0, stairs.length).map(value -> IntStream.of(stairs[value]).sum()).sum();
        return 20 * Arrays.stream(stairs).mapToInt(stair -> IntStream.of(stair).sum()).sum();
    }
